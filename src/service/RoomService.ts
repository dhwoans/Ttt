...
class RoomService {
  ...
  removePlayer(
    ...
    return this.roomRegistry.removePlayer(roomId, connId);
  }
  ...
  joinPlayer(
    ...
    return this.roomRegistry.joinPlayer(roomId, connId, nickname, avatar);
  }
  ...
  readyPlayer(
    ...
    return this.roomRegistry.readyPlayer(roomId, connId, status);
  }
  ...
  gameStart(roomId: RoomId): SuccessResponse<void> | FailureResponse {
    ...
    if (result.success && result.message) {
      ...
      if (room.isFull()) {
        ...
        const playerIds = room.getAllPlayersData().map((player) => player.connId);
