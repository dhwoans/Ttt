...
class SocketIOManager {
    ...
    gameFlow;
    publisher;
    constructor(server, roomService, redis) {
        ...
        this.presence = new RoomPresenceGateway(roomService, this.errors, this.publisher);
        this.gameFlow = new GameFlowService(roomService, this.errors, this.publisher);
    }
    /**
    ...
    init() {
        ...
        console.log("[SocketIOManager] Socket.IO is ready and listening for connections");
    ...
    setupEventListeners() {
        ...
        this.io.on("connection", async (socket) => {
            ...
            socket.on("READY", (data) => {
                console.log(`[SocketIOManager] ✅ READY event from ${socket.id}:`, data);
            ...
            socket.on("disconnect", (reason) => {
                console.log(`[SocketIOManager] 🔌 Disconnect: ${socket.id}, reason: ${reason}`);
                const leaveResult = this.presence.leave(socket, false);
                if (!leaveResult.success) {
                    console.log("[SocketIOManager] No room/user data found for disconnecting socket");
                    return;
                }
                this.gameFlow.onRoomStateChanged(leaveResult.roomId);
            });
